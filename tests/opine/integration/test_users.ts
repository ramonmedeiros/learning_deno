...
import { start_app } from "../../../opine/app.ts";
...
Rhum.testPlan("Opine", () => {
  ...
  Rhum.testSuite("Testing index", () => {
    Rhum.testCase("GET /users empty", async () => {
    ...
    Rhum.testCase("POST /users wrong parameters", async () => {
      const response = await fetch("http://localhost:3000/users", {
        method: "POST",
        headers: {
    ...
    Rhum.testCase("POST /users success", async () => {
      const response = await fetch("http://localhost:3000/users", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ "user": "test1", "password": "pass1" }),
      }).then((response) => response);
    ...
    Rhum.testCase("GET /users with one return", async () => {
      const response = await fetch("http://localhost:3000/users", {
        method: "GET",
      }).then((response) => response);
      // assert return
